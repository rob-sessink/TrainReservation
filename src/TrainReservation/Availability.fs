module TrainReservation.Availability
...
let calculateCapacity: CalculateCapacity =
    ...
    fun maximumAllowed unitTotal unitAllocated ->
        ...
        // 70 - 67 = 3
        let allocatable = (Percentage.value maximumAllowed) - current

        // 30.m * (3.m / 100.m) -> rounded down = 0
        let unitAllocatable =
            total * (allocatable / 100.m)
            |> Math.Round
            |> int
...
let availableSeatsForCoach coach seats =
    seats |> List.filter (fun s -> s.SeatDetail.Coach = coach && s.SeatDetail.BookingReference = "")
...
let toAvailability requested capacity: Availability =
    match capacity with
    | c when c.Allocatable > Percentage 0m && c.UnitAllocatable < requested -> Unavailable c
    | c when c.Allocatable > Percentage 0m && c.UnitAllocatable >= requested -> Available c
...
let groupedPerCoachAllotment: AllotmentStrategy =
    fun request allocationSettings trainInformation ->
        ...
        let coachesCapacity = calculateCoachesCapacity allocationSettings trainInformation
